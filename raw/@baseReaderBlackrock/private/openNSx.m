...
NSx.MetaTags.openNSxver = '4.1.3.0';
...
if strcmp(ReadData, 'read')
    ...
    if elecReading
        if ~all(ismember(Chan,[NSx.MetaTags.ChannelID]))
        ...
        if ~all(ismember(Chan,[NSx.MetaTags.ChannelID]))
            fprintf('The recorded channels were: %s\n', num2str([NSx.ElectrodesInfo.ElectrodeID]));
            fprintf(2,'Data was not read.\n');        
            fclose(FID);
            if nargout; varargout{1} = []; end
            return;
    ...
    fseek(FID, (find(NSx.MetaTags.ChannelID == min(Chan))-1) * 2, 'cof');
...
%% Fix a bug in Central that doesn't populate DataPoints
if strcmpi(ReadData, 'read')
    if (NSx.MetaTags.DataPoints == 0) && (size(NSx.Data, 2) ~= 0)
