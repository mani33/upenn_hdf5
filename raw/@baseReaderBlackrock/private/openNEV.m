...
%
%   'overwrite':  Option to overwrite the MAT file. 'overwrite' will overwrite 
%                 the existing MAT file. 'nowrite' will skip the
%                 overwrite. 'ask' will prompt for input
%                 DEFAULT : 'ask'
%
...
for i=1:length(varargin)
    switch lower(varargin{i})
        ...
        case 'nowave'
            Flags.NoWave = varargin{i};
        case {'overwrite','nowrite'}
            Flags.Overwrite = varargin{i};
        case 'nowarning'
...
if ~isfield(Flags, 'NoWave');        Flags.NoWave = 'yeswave'; end;
if ~isfield(Flags, 'Overwrite');     Flags.Overwrite = 'ask'; end;
...
if exist(matPath, 'file') == 2 && strcmpi(Flags.NoMAT, 'yesmat') && strcmpi(Flags.NoWave,'yeswave')
...
if strcmpi(Flags.ReadData, 'read')
    ...
    if strcmpi(Flags.NoWave,'yeswave')
...
if strcmpi(Flags.SaveFile, 'save')
    if exist(matPath, 'file') == 2
        ...
        if strcmpi(Flags.Overwrite,'overwrite')
            overWrite = 'y';
        elseif strcmpi(Flags.Overwrite,'nowrite')
            overWrite = 'n';
        else
        ...
        if strcmpi(overWrite, 'y')
            ...
            disp('MAT File was not overwritten.');
        end
    else
        disp('Saving MAT file. This may take a few seconds...');
        save(matPath, 'NEV', '-v7.3');
